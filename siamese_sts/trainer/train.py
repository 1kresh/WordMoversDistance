...
def train_model(model, optimizer, dataloader, max_epochs, config_dict):
    ...
    for epoch in tqdm(range(max_epochs)):
        ...
        val_acc, val_loss = evaluate_dev_set(
            model, criterion, dataloader, config_dict, device
        )
...
def attention_penalty_loss(annotation_weight_matrix, penalty_coef, device):
    ...
    ## compute loss
    loss = (penalty_coef * penalty / batch_size).type(torch.float32)
